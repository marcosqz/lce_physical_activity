...
# Plot a subset of the climate, population and regional boundaries to check
# that all coordinates system match

# Extension of the països catalans (subset of the climate data)
raster_plot <- crop(raster_temp, extent(-1, 5, 38, 43))
polygon_temp <- rasterToPolygons(raster_plot, dissolve = TRUE)
outline_temp <- sf::st_as_sf(polygon_temp) ## Convert SpatialPolygons to a format usable by ggplot2
rm(raster_plot, polygon_temp)

# Regional boundaries
shp_zones <- st_read('indata/other/Europe_regions/Europe_regions.shp')
outline_boundaries <- sf::st_as_sf(shp_zones)
rm(shp_zones)

pdf(paste0("outdata/plot/data_resolutions_ppcc.pdf"), width = 20, height = 28)
plot(shp_popu[50000:600000,]$geometry, xlim = c(-0.5, 3), ylim = c(38, 42))
plot(outline_boundaries, col = NA, border = "red", add = TRUE)
plot(outline_boundaries, col = NA, border = "blue", add = TRUE)
legend(2, 39, 
       c("population (geostat)", "gisco (shapefile)", "era5land (temperature)"), 
       col = c("black", "red", "blue"), lty = 1, lwd = 2, cex = 1.5, bty = "n")
dev.off()

rm(outline_boundaries, outline_boundaries)
